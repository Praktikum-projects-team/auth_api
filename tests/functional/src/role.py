...
class Role:
    def test_roles_all(self):
        ...
        for _ in range(3):
            ...
            make_post_request(ROLES_URL, body=role_data['name'])
    ...
    def test_role_create(self):
        ...
        role_data = get_role_data()
        resp = make_post_request(ROLES_URL, body=role_data['name'])
    ...
    def test_role_info(self):
        ...
        assert resp.status == HTTPStatus.OK, 'Wrong status code'
        assert resp.data['id'] == role_data['id'], 'Wrong id'
        assert resp.data['name'] == role_data['name'], 'Wrong name'
    ...
    def test_role_update(self):
        ...
        role_data = get_role_data()
        make_post_request(ROLES_URL, body=role_data['name'])
        resp_all_roles = make_get_request(ROLES_URL)
        role_id = get_role_id_by_name(resp_all_roles, role_data['name'])

        resp = make_put_request(f'{ROLES_URL}/{role_id}', body=role_data['name'])
    ...
    def test_role_update_duplicate(self):
        ...
        make_put_request(f'{ROLES_URL}/{role_id}', body=role_data['name'])
        resp = make_put_request(f'{ROLES_URL}/{role_id}', body=role_data['name'])
    ...
    def test_role_update_not_found(self):
        ...
        role_data = get_role_data()
        resp = make_put_request(f'{ROLES_URL}/{role_data["id"]}', body=role_data['name'])
